...
with st.sidebar:
    model = st.selectbox("Choose Your model", ("mistral","llama3.1"))
    if model == "mistral":
        llm = ChatOllama(
...
def embed_file(file):
    ...
    file_path = f"./.cache/private_files_ollama/{file.name}"
    with open(file_path, "wb") as f:
        f.write(file_content)
    cache_dir = LocalFileStore(f"./.cache/private_embeddings_ollama/{file.name}")
